...
##Explore Read Counts for combined DNA/RNA physeq
...
##Exploring Read Counts for DNA samples physeq
...
##Exploring Read Counts for RNA samples physeq
...
```{r}
# make histogram of raw reads combined
raw_TotalSeqs_df %>%
...
```{r}
# make histogram of raw reads DNA
raw_TotalSeqs_df_DNA %>%
  ggplot(aes(x = TotalSeqs)) + 
  geom_histogram(bins = 15) + 
  scale_x_continuous(limits = c(2000, 10000)) + 
...
```{r}
# make histogram of raw reads RNA
raw_TotalSeqs_df_RNA %>%
  ggplot(aes(x = TotalSeqs)) + 
  geom_histogram(bins = 15) + 
  scale_x_continuous(limits = c(2000, 10000)) + 
...
min(sample_sums(midroot_physeq_DNA))
min(sample_sums(midroot_physeq_RNA))
...
scaled_rooted_physeq_DNA <- midroot_physeq_DNA %>% scale_reads(round = "matround")
scaled_rooted_physeq_RNA <- midroot_physeq_RNA %>% scale_reads(round = "matround")

# Calculate the read depth 
scaled_TotalSeqs_df <- scaled_rooted_physeq %>% sample_sums() %>% data.frame()
scaled_TotalSeqs_df_DNA <- scaled_rooted_physeq_DNA %>% sample_sums() %>% data.frame()
scaled_TotalSeqs_df_RNA <- scaled_rooted_physeq_RNA %>% sample_sums() %>% data.frame()

colnames(scaled_TotalSeqs_df)[1] <-"TotalSeqs"
colnames(scaled_TotalSeqs_df_DNA)[1] <-"TotalSeqs"
colnames(scaled_TotalSeqs_df_RNA)[1] <-"TotalSeqs"

# Inspect
head(scaled_TotalSeqs_df)
head(scaled_TotalSeqs_df_DNA)
head(scaled_TotalSeqs_df_RNA)
```
...
min_seqs_DNA <- min(scaled_TotalSeqs_df_DNA$TotalSeqs); min_seqs
max_seqs_DNA <- max(scaled_TotalSeqs_df_DNA$TotalSeqs); max_seqs
max_seqs_DNA - min_seqs_DNA

min_seqs_RNA <- min(scaled_TotalSeqs_df_RNA$TotalSeqs); min_seqs
max_seqs_RNA <- max(scaled_TotalSeqs_df_RNA$TotalSeqs); max_seqs
...
scaled_TotalSeqs_df %>%
  ...
  labs(title = "Scaled Sequencing Depth Combined") + 
  theme_classic()

scaled_TotalSeqs_df_DNA %>%
  ggplot(aes(x = TotalSeqs)) + 
  geom_histogram(bins = 50) + 
  scale_x_continuous(limits = c(0, 10000)) + 
  labs(title = "Scaled Sequencing Depth DNA") + 
  theme_classic()

scaled_TotalSeqs_df_RNA %>%
  ggplot(aes(x = TotalSeqs)) + 
  geom_histogram(bins = 50) + 
  scale_x_continuous(limits = c(0, 10000)) + 
  labs(title = "Scaled Sequencing Depth at RNA") + 
  theme_classic()
...
# Plot the ordination 
soren_station_pcoa_combined <- plot_ordination(
  physeq = scaled_rooted_physeq,
  ordination = scaled_soren_pcoa_combined,
  color = "lakesite",
  title = "Sorensen PCoA combined samples ") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = lakesite_colors) + 
  theme_bw()

soren_station_pcoa_DNA <- plot_ordination(
  physeq = scaled_rooted_physeq_DNA,
  ordination = scaled_soren_pcoa_DNA,
  color = "lakesite",
  title = "Sorensen PCoA DNA samples ") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = lakesite_colors) + 
  theme_bw()

soren_station_pcoa_RNA <- plot_ordination(
  physeq = scaled_rooted_physeq_RNA,
  ordination = scaled_soren_pcoa_RNA,
  color = "lakesite",
  title = "Sorensen PCoA RNA samples ") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = lakesite_colors) + 
  theme_bw()
...
# Show the plot 
soren_station_pcoa_combined
soren_station_pcoa_DNA
soren_station_pcoa_RNA
```
...
soren_station_pcoa <- plot_ordination(
  ...
  color = "lakesite",
  shape = "limnion",
  title = "Sorensen PCoA by limnion") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = lakesite_colors) + 
  theme_bw()
...
soren_station_pcoa
```
 The Sorensen PCoA gives us an idea of the presence/absence between samples specifically between lakesite for the above. Since we dissected the samples by DNA and RNA the following interpretation is going be on those respective samples. The DNA sample has 47% of the variation explained by the principle axis while the RNA sample has 40% variation explained by the principle axis. The lake surface samples heavily cluster together for both DNA and RNA indicating that the active/total taxa cluster in a similar manner.

Bray

```{r}
# Calculate the BC distance
scaled_BC_pcoa_combined <- 
  ordinate(
    physeq = scaled_rooted_physeq,
    method = "PCoA",
    distance = "bray")
...
# Plot the PCoA
bray_station_pcoa_combined <- 
  plot_ordination(
    physeq = scaled_rooted_physeq,
    ordination = scaled_BC_pcoa_combined,
    color = "lakesite",
    title = "Bray-Curtis PCoA Combined") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20,21)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()

bray_station_pcoa_DNA <- 
  plot_ordination(
    physeq = scaled_rooted_physeq_DNA,
    ordination = scaled_BC_pcoa_DNA,
    color = "lakesite",
    title = "Bray-Curtis PCoA DNA") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20,21)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()

bray_station_pcoa_RNA <- 
  plot_ordination(
    physeq = scaled_rooted_physeq_RNA,
    ordination = scaled_BC_pcoa_RNA,
    color = "lakesite",
    title = "Bray-Curtis PCoA RNA") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20,21)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
...
bray_station_pcoa <- 
  plot_ordination(
    ...
    color = "lakesite",
    shape = "season",
    title = "Bray-Curtis PCoA by season") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20,21)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
...
bray_station_pcoa <- 
  plot_ordination(
    ...
    color = "lakesite",
    shape = "limnion",
    title = "Bray-Curtis PCoA by limnion") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20,21)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
...
# Plot the PCoA
wUNI_station_pcoa_combined <- 
  plot_ordination(
    physeq = scaled_rooted_physeq,
    ordination = scaled_wUNI_pcoa_combined,
    color = "lakesite",
    shape = "lakesite",
    title = "Weighted Unifrac PCoA Combined") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()

wUNI_station_pcoa_DNA <- 
  plot_ordination(
    physeq = scaled_rooted_physeq_DNA,
    ordination = scaled_wUNI_pcoa_DNA,
    color = "lakesite",
    shape = "lakesite",
    title = "Weighted Unifrac PCoA DNA") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()

wUNI_station_pcoa_RNA <- 
  plot_ordination(
    physeq = scaled_rooted_physeq_RNA,
    ordination = scaled_wUNI_pcoa_RNA,
    color = "lakesite",
    shape = "lakesite",
    title = "Weighted Unifrac PCoA RNA") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17,18, 19, 20)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
...
wUNI_station_pcoa <- 
  plot_ordination(
    ...
    color = "lakesite",
    shape = "season",
    title = "Weighted Unifrac PCoA by season") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
...
wUNI_station_pcoa <- 
  plot_ordination(
    ...
    color = "lakesite",
    shape = "limnion",
    title = "Weighted Unifrac PCoA by limnion") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
...
```{r}
# Plot the PCoA
wUNI_station_nmds_combined <- 
  plot_ordination(
    physeq = scaled_rooted_physeq,
    ordination = scaled_wUNI_nmds_combined,
    color = "lakesite",
    shape = "nuc_acid_type",
    title = "Weighted Unifrac NMDS combined") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()

wUNI_station_nmds_DNA <- 
  plot_ordination(
    physeq = scaled_rooted_physeq_DNA,
    ordination = scaled_wUNI_nmds_DNA,
    color = "lakesite",
    shape = "nuc_acid_type",
    title = "Weighted Unifrac NMDS DNA") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()

wUNI_station_nmds_RNA <- 
  plot_ordination(
    physeq = scaled_rooted_physeq_RNA,
...
wUNI_station_nmds <- 
  plot_ordination(
    ...
    color = "lakesite",
    shape = "season",
    title = "Weighted Unifrac NMDS by season") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
...
wUNI_station_nmds <- 
  plot_ordination(
    ...
    color = "lakesite",
    shape = "limnion",
    title = "Weighted Unifrac NMDS by limnion") +
  geom_point(size=5, alpha = 0.5, aes(color = lakesite)) +
  scale_shape_manual(values = c(15,16,17)) +
  scale_color_manual(values = c(lakesite_colors)) + 
  theme_bw()
